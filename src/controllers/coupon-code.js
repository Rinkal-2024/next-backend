...
const getHighestDiscountCoupon = async (req, res) => {
    try {
        const couponCode = await CouponCode.findOne().sort({ discount: -1 }).limit(1);
        if (!couponCode) {
            return res.status(404).json({ success: false, message: "No coupon codes found" });
        }
